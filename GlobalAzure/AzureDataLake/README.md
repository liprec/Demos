...
1. Open the `01. Script.usql` script file and copy the content to your own version
...
1. Opern the Cloud Explorer in Visual Studio
...
1. Open or create a copy of the  previous query files which uses the single file query (`01. Script.usql`) file
2. Add two references to the newly registered assemblies as the first lines of the query file
```
    REFERENCE ASSEMBLY [Newtonsoft.Json];
    REFERENCE ASSEMBLY [Microsoft.Analytics.Samples.Formats];
```
3. Add a first transformation to parse the JSON from the extractor. Hint: `Microsoft.Analytics.Samples.Formats.Json.JsonFunctions.JsonTuple()`
4. Add an second transformation to return the individual dictonary returned by the `JsonTuple` method as columns
5. Output all the columns as a CSV file
...
3. Create a method (`public string GetBuilding()`)
4. Return the first part of the `split` of the string (Hint: `string.Split(',')`)
5. Alter the query to use the custom method for location
6. Test the result by submitting the query as a job
